...
def build_dataloader(args, tranforms=None):
    logger = logging.getLogger("DGCMIA.dataset")

    num_workers = args.num_workers
    dataset = __factory[args.dataset_name](root=args.root_dir)
    # dataset.train =[dataset.train[i] for i in range(1000)]
    num_classes = len(dataset.train_id_container)
    args.ndata = dataset.train.__len__()
    ...
    if args.training:
        ...
        train_transforms1 = build_transforms(img_size=args.img_size,
                                    is_train=False)
